...
if __name__ == '__main__':
    ...
    audio, _ = librosa.load(url, sr=processor.feature_extractor.sampling_rate)

    inputs = processor(text=prompt, audios=audio, return_tensors="np")

    for key,value in inputs.item():
    ...
    # Generate
    generate_ids = model.generate(**inputs, max_length=30, use_cache=False)
    print(generate_ids)
